...
3. **生成反馈内容**:
    ...
    **任务**：根据 `task_state.final_status` 生成反馈
    ...
    3. **生成反馈**:
       ...
       - 体现用户人设和当前对话策略
       - **证据验证准则（最高优先级）**：
         - **实证优先**：若任务目标涉及“生成文件”、“编写报告”或“输出代码”，必须核对 `file_context`。
         - **拒绝幻觉**：若 Agent 声称已输出文件，但 `file_context` 中不存在该文件或内容为空，**严禁**将其视为任务完成。
         - **质疑反馈**：在这种情况下，应在 `feedback` 中直接质疑（例如：“你说文件写好了，但我没看到，请检查路径。”），并确保 `judgment` 为 `in_progress`。
